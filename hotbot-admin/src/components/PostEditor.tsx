...
const N8N_CREATE_PATH = (
  process.env.NEXT_PUBLIC_N8N_WEBHOOK_BLOG_CREATE || "hotbotstudios-blog-create"
).replace(/^\//, "");
...
export const AD_TOPICS: { value: AdTopic; label: string }[] = [
...
type SubmitMode = "n8n" | "direct";
type Status = "idle" | "sending" | "success" | "error";

interface PostEditorProps {
  /** Initial form values (for edit mode). Defaults to EMPTY_FORM. */
  initial?: Partial<PostFormData>;
  /** "new" shows writing instructions + N8N mode. "edit" defaults to direct save. */
  mode?: "new" | "edit";
  /** Called after successful save so parent can redirect or show a message. */
  onSuccess?: (result: { url?: string; message?: string; slug: string }) => void;
}

export function PostEditor({ initial = {}, mode = "new", onSuccess }: PostEditorProps) {
  const [form, setForm] = useState<PostFormData>({ ...EMPTY_FORM, ...initial });
  const [submitMode, setSubmitMode] = useState<SubmitMode>(mode === "edit" ? "direct" : "n8n");
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (submitMode === "n8n" && !form.content.trim() && !form.writingInstructions.trim()) {
      alert("Provide content or writing instructions for Claude.");
      return;
    }
    ...
    try {
      if (submitMode === "n8n") {
        // Route through N8N → Claude for AI-enhanced content
        ...
        if (!res.ok || !data.success) {
          setStatus("error");
          setResult({ error: data.error || data.message || "N8N workflow error." });
          return;
        }
        setStatus("success");
        const successResult = {
          url: data.url || `${MAIN_SITE}/blog/${form.slug}`,
          message: data.message || "Post published via N8N + Claude!",
          slug: form.slug,
        };
        setResult(successResult);
        onSuccess?.(successResult);
      } else {
        // Direct save via server-side proxy (bypasses Claude)
        const res = await fetch("/api/publish", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ action: "publish", post: postPayload }),
        });
        const data = await res.json() as { success?: boolean; url?: string; slug?: string; error?: string };
        if (!res.ok || !data.success) {
          setStatus("error");
        ...
        const successResult = {
          url: data.url || `${MAIN_SITE}/blog/${form.slug}`,
          message: "Post saved directly to hotbotstudios.com!",
      ...
      setStatus("error");
      setResult({ error: submitMode === "n8n" ? "Could not reach N8N." : "Could not reach publish API." });
    }
  }
    ...
    <div>
      ...
      {status === "success" && result && (
        <div className="mb-6 p-5 rounded-2xl bg-green-500/10 border border-green-500/25">
          ...
          {result.url && (
            <a href={result.url} target="_blank" rel="noopener noreferrer" className="text-green-300 text-sm underline">
              {result.url} ↗
            </a>
          )}
        </div>
      ...
      {/* Submit mode toggle */}
      <div className="mb-6 flex items-center gap-2 p-1 rounded-xl bg-white/[0.03] border border-white/[0.07] w-fit">
        <button
          type="button"
          onClick={() => setSubmitMode("n8n")}
      ...
      <p className="text-slate-600 text-xs mb-6 -mt-4">
        {submitMode === "n8n"
          ? "Sends to N8N → Claude AI will generate/enhance content → publishes live."
          : "Saves your content as-is directly to hotbotstudios.com/blog. No AI processing."}
      </p>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <Field label="Inline Ad Topic" hint="Which HotBot service should inline ads promote?">
            <select value={form.adTopic} onChange={(e) => updateField("adTopic", e.target.value as AdTopic)} className={inputClass}>
        ...
        {/* Writing instructions — N8N mode only */}
        {submitMode === "n8n" && (
          <Field
            label="Writing Instructions for Claude"
            hint="Tell Claude the topic, tone, target audience, keywords, length, structure. If you provide full content below, Claude will enhance it instead."
          >
            <textarea
              value={form.writingInstructions}
              onChange={(e) => updateField("writingInstructions", e.target.value)}
              placeholder={`Example:\nWrite a 1200-word post about local SEO for US dentists in 2026.\nTone: authoritative but approachable.\nInclude: Google Business Profile tips, review strategies, local content ideas.\nTarget keywords: "dental SEO 2026", "local SEO for dentists"\nEnd with a CTA to book a free SEO audit with HotBot Studios.`}
              rows={7}
              className={inputClass}
            />
          </Field>
        ...
        <div>
          <div className="flex items-center justify-between mb-1.5">
            <label className="text-sm font-semibold text-slate-300">
              Content (HTML){submitMode === "n8n" ? " — Optional" : " *"}
            </label>
            <button
              type="button"
          ...
          {submitMode === "n8n" && (
            <p className="text-slate-600 text-xs mb-2">
              If provided, Claude enhances this. If blank, Claude generates from instructions above.
            </p>
          )}
          {preview ? (
            <div
              className="w-full px-4 py-3 rounded-xl bg-white/[0.05] border border-white/[0.1] text-slate-300 text-sm prose prose-invert max-w-none min-h-[200px] overflow-auto"
              dangerouslySetInnerHTML={{ __html: form.content || "<p class='text-slate-600'>Nothing to preview yet.</p>" }}
            />
          ) : (
            <textarea
              value={form.content}
        ...
        {/* Tags + Author */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Field label="Tags" hint="Comma-separated: seo, google, marketing">
            <input
              type="text"
              value={form.tags}
        ...
        <div className="pt-2">
          <button
            ...
            className={`w-full py-4 rounded-2xl font-black text-base disabled:opacity-50 disabled:cursor-not-allowed text-white transition-all duration-200 hover:shadow-xl ${
              submitMode === "n8n"
          ...
          >
            {status === "sending" ? (
              <span className="flex items-center justify-center gap-3">
                ...
                {submitMode === "n8n" ? "Sending to N8N → Claude is writing..." : "Saving..."}
              </span>
            ) : submitMode === "n8n" ? (
              "🚀 Send to N8N & Publish"
            ) : (
              "⚡ Save Directly to Blog"
            )}
          </button>
        </div>
      </form>
