...
export default function EditPostPage() {
  ...
  useEffect(() => {
    if (!slug) { router.replace("/backdrop/dashboard"); return; }
    ...
    fetch(`/api/blog/publish?secret=${encodeURIComponent(secret)}&slug=${encodeURIComponent(slug)}`)
      ...
      .then((d: { exists: boolean; post: BlogPost | null }) => {
        if (!d.exists || !d.post) { router.replace("/backdrop/dashboard"); return; }
        const p = d.post;
        setPost(p);
        setForm({
    ...
    <div className="flex flex-col min-h-screen">
      ...
      >
        <div className="flex items-center gap-3">
          <Link href="/backdrop/dashboard" className="text-slate-400 hover:text-white transition-colors text-sm">← Dashboard</Link>
