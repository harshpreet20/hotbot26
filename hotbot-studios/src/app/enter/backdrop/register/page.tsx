...
const ROLES = [
  ...
  { value: "manager",     label: "Manager",      desc: "View CRM data & reports" },
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      const res  = await fetch("/api/blog/users/register", {
        ...
        body: JSON.stringify({ username: username.trim(), email: email.trim(), requestedRole: role }),
      });
        ...
        {success ? (
          <div className="rounded-2xl p-6 text-center space-y-3"
            ...
            <p className="text-slate-400 text-xs leading-relaxed">
              The admin has been notified and will review your request. You&apos;ll hear back at <strong className="text-slate-300">{email}</strong>.
            </p>
          ...
          <form onSubmit={handleSubmit} noValidate className="rounded-2xl p-6 space-y-4"
            ...
            {/* Username */}
            <div>
              <label htmlFor="username" className="block text-xs font-medium text-slate-400 mb-1.5 uppercase tracking-wider">Username</label>
              <input id="username" type="text" autoComplete="username" value={username}
                ...
                style={inputStyle} placeholder="yourname"
